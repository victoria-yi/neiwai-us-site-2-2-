...
export default function CompleteTheSetPDP({ briefs }: CompleteTheSetPDPProps) {
  ...
  return (
    <section className="bg-warm-white/70 pt-10 pb-16 lg:py-24">
      <div className="px-6 lg:px-20 max-w-[1440px] mx-auto">
        {/* Header — centered, refined typography */}
        <FadeIn>
          <div className="text-center mb-12 lg:mb-16">
            <h2 className="font-display text-[22px] lg:text-[34px] font-light text-ink leading-tight tracking-[0.02em]">
              Complete the set
            </h2>
            <Link
              href="/briefs"
              className="inline-flex items-center gap-2 mt-[14px] lg:mt-4 font-body text-[10px] lg:text-[11px] tracking-[0.14em] uppercase text-ink hover:text-accent transition-colors duration-300 group"
            >
              Shop Briefs
        ...
        {/* Product cards — mobile: larger frames + smaller type; desktop: unchanged */}
        <div className="grid grid-cols-2 gap-4 lg:gap-8">
          {briefs.map((p, i) => (
            <FadeIn key={p.id} delay={i * 0.12}>
              <Link href={`/${p.category}/${p.slug}`} className="group block w-full lg:w-[85%] lg:max-w-[85%] lg:mx-auto">
